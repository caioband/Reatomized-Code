...
Handler.FirstPerson.Modes.Calm.Idle.Y = .3
...
Handler.FirstPerson.Modes.Panting.Idle.Y = .5
...
function Handler:FirstPersonCamera(dt)
    ...
    if Handler.Limiter >= 1/60 then
        ...
        Vel = NumLerp(Handler.CurrentVel,GetVelMag(Character.HumanoidRootPart), .25)
...
function Handler:CreateNewCamera(CameraMode : string)
    ...
    if self.RenderStepped then
        RunService:UnbindFromRenderStep(self.RenderStepped.Name)
        self:Reset()
    end
