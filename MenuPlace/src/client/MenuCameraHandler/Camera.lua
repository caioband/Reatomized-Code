...
Handler.TransictionConcluded = false
Handler.SToG = nil
...
function GetVelMag(Part) : number
	return math.round(Vector3.new(Part.AssemblyLinearVelocity.X, Part.AssemblyLinearVelocity.Y, Part.AssemblyLinearVelocity.Z).Magnitude)
end
...
function GetSwayVal(x:number, y:number, CCamera) : CFrame
	return CFrame.new(Vector3.new(x, y, 0), Vector3.new(x*.95, y*.95, -10)) + CCamera.CFrame.Position
end
...
function Handler:StartMenuScenes()
    local self = GetSelf()
    local Music = workspace.Musics["When Stars Collide"] :: Sound
    Music.Looped = true
    Music:Play()
...
function FadeEffect(Transparency)
    local Tween1 = TweenService:Create(Player.PlayerGui.ScreenGui.TransactionFrame, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),{BackgroundTransparency = Transparency})
    Tween1:Play()
end
...
function NeonEuclideanGarage(NewCamera)
    ...
    local Tween2 = TweenService:Create(NewCamera, TweenInfo.new(Time2, Enum.EasingStyle.Linear),{CFrame = CameraPoses.Transaction2.CFrame})
    NewCamera.CFrame = CameraPoses.Main3.CFrame
    Tween2:Play()
    task.delay(Time2 * .8, function()
        FadeEffect(0)
    end)
...
function Handler:NonEuclideanScene()
    ...
    local Distance2 = (CameraPoses.Main3.Position - CameraPoses.Transaction2.Position).Magnitude
    local Time2 = Distance2/3
    local Tween1 = TweenService:Create(NewCamera, TweenInfo.new(Time, Enum.EasingStyle.Linear),{CFrame = CameraPoses.Main2.CFrame})
    Tween1:Play()
    task.wait(Time * .999)
    self.SToG = true
end
